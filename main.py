...
def update_block_period():
    ...
    if 'block_period' not in data:
        return jsonify({"error": "Missing block_period"}), 400
    try:
        block_date = dt.strptime(data['block_period'], "%Y-%m-%d").date()
        settings = settings_manager()
        settings.settings.block_period = block_date
        # Recalculate and save blocked OSV
        service = start_service()
        transactions = service.data['transaction_key']
        osv_block = prototype.generate_osv_up_to_block(transactions, block_date)
        prototype.save_blocked_osv(osv_block)
        settings.load()  # Persist if needed (extend load to save)
        return jsonify({"success": True, "block_period": data['block_period']})
    except ValueError:
...
@app.route("/api/remainders/<date>", methods=['GET'])
def get_remainders(date):
    try:
        target_date = dt.strptime(date, "%Y-%m-%d").date()
        service = start_service()
        transactions = service.data['transaction_key']
        settings = settings_manager().settings
        remainders = prototype.generate_osv_with_block(transactions, target_date, settings.block_period)
